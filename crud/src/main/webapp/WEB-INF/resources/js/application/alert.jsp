...
<script>
	...
	function confirmModal(callback, title, content) {
		...
		var contentModal = '<fmt:message key="js.alert.confirmation" />';
		...
		html += '<button class="btn btn-danger btnCancel" data-dismiss="modal">Nie</button>';
		html += '</div>';
		html += '</div>';
		html += '</div>';
		...
		var titleModal = '<fmt:message key="js.alert.confirm.title" />';
	...
	function errorPopup(elements) {
		...
		elements
				.each(function() {
					...
					// console.log("seelct2: "+select2.length)
					// console.log("msg: "+msg)
					// console.log("class: "+parent.attr("class"))
					// console.log("znalezionych errorElementow: "+elements.length)
					...
					parent.find("input").each(function() {
						...
						if (input.data("select2") !== undefined) {
							elements = select2;
							// console.log("znaleziono selecta 2 z klasami:
							// "+input.attr("class"))
					...
					if (elements.hasClass("check")) {
						var label = elements.parent().find(".check-label");
						label.addClass("errorFieldCheck");
					}
