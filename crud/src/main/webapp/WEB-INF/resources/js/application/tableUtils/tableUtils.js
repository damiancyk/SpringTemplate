...
function getTable() {
	var tables = $("table");
	var table = $('div.dx-datagrid-rowsview table.dx-datagrid-table').first();
	if (table.length == 0) {
		table = $('table').first();
	}
...
function replaceDisplaytagParams(url, options, table) {
	...
	if (indexId >= 0 && indexIds >= 0) {
		console.error("Error - both parameters: ID and IDS are defined");
		return;
	}
...
function getNumberOfChecked(table, params) {
	...
	if (typeof params.oneRow !== "undefined") {
		if (positions > 1) {
			alertInfo(tableUtilsLocale.selectOnlyOneRow);
	...
	if (positions == 0) {
		if (typeof params.noErr !== "undefined" && params.noErr == true) {
			...
			alertInfo(tableUtilsLocale.noSelectedColumn);
